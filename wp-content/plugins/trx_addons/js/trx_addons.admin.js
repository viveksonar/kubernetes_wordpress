	...
	jQuery(document).ready(function() {
			...
			.on('change', 'select[class*="post_type"],select[class*="taxonomy"]', function () {
				var refresh_obj = jQuery(this),
					...
					cat_flds = refresh_post_type
						? refresh_obj.parents('.vc_shortcode-param').nextAll('[data-vc-shortcode-param-name*="taxonomy"]').find('select')
		...
		jQuery('body')
			.on('change', 'select[name*="post_type"],select[name*="taxonomy"]', function () {
				var refresh_obj = jQuery(this),
					refresh_post_type = (refresh_obj.attr('name').indexOf('post_type') != -1),
					cat_flds = refresh_post_type
						? refresh_obj.parents('.siteorigin-widget-field,[class*="widget_field_type_"]')
							.nextAll('.widget_field_type_select')
							.filter(function () {
								return jQuery('select[name*="taxonomy"]', jQuery(this)).length > 0;
							})
							.find('select')
						: jQuery(this).parents('.siteorigin-widget-field,[class*="widget_field_type_"]').next().find('select');
				if (cat_flds.length > 0) {
					jQuery.each(cat_flds, function(index, cat_fld){
						cat_fld = jQuery(cat_fld);
						var cat_lbl = cat_fld.parents('.siteorigin-widget-field,[class*="widget_field_type_"]').find('.siteorigin-widget-field-label,label.widget_field_title');
		...
		jQuery('body').on('change', 'select', function () {
			var selector = jQuery(this),
				parent = selector.data('setting') !== undefined
							? selector.closest('.elementor-control-content')
							: selector.parent();
