...
class trx_addons_widget_contacts extends TRX_Addons_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_contacts', 'description' => esc_html__('Contacts - logo and short description, address, phone and etc', 'trx_addons'));
	...
	function widget($args, $instance) {
		...
		if (!empty($logo)) {
			...
			if (!empty($logo)) {
				...
				$logo = '<img src="'.esc_url($logo).'" alt="' . esc_attr__("Contact's logo", 'trx_addons') . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
			}
			// Logo for Retina
			if (!empty($logo_retina) && trx_addons_get_retina_multiplier() > 1) {
				...
				if (!empty($logo_retina)) {
					$logo = '<img src="'.esc_url($logo_retina).'" alt="'.esc_attr__("Contact's logo for Retina", 'trx_addons').'"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'>';
				}
			}
		}
		...
		$bdate = isset($instance['bdate']) ? $instance['bdate'] : '';
		...
		trx_addons_get_template_part(array(
			TRX_ADDONS_PLUGIN_WIDGETS . 'contacts/tpl.'.trx_addons_esc($style).'.php',
			TRX_ADDONS_PLUGIN_WIDGETS . 'recent_news/tpl.contacts-labels.php'
		),
									'trx_addons_args_widget_contacts', 
									apply_filters('trx_addons_filter_widget_args',
												array_merge($args, compact('style', 'title', 'logo', 'logo_retina', 'description', 'content',
																			'email', 'columns', 'name', 'bdate', 'address', 'phone', 'site', 'socials', 'googlemap',
	...
	function update($new_instance, $instance) {
		...
		$instance['name'] = wp_kses_data($new_instance['name']);
		$instance['bdate'] = wp_kses_data($new_instance['bdate']);
		$instance['site'] = wp_kses_data($new_instance['site']);
		$instance['new_window'] = wp_kses_data($new_instance['new_window']);
	...
	function form($instance) {
		...
		$this->show_field(array('name' => 'style',
			...
			'options' => trx_addons_components_get_allowed_layouts('widgets', 'contacts'),
			'type' => 'select'));
		...
		$this->show_field(array('name' => 'name',
								'title' => __('Name:', 'trx_addons'),
								'value' => $instance['name'],
								'type' => 'text'));

		$this->show_field(array('name' => 'bdate',
								'title' => __('Date of birth:', 'trx_addons'),
								'value' => $instance['bdate'],
								'type' => 'text'));
		...
		$this->show_field(array('name' => 'site',
								'title' => __('Website:', 'trx_addons'),
								'value' => $instance['email'],
								'type' => 'text'));

		$this->show_field(array('name' => 'new_window',
								'title' => __('Open in new window:', 'trx_addons'),
								'value' => (int) $instance['new_window'],
								'type' => 'checkbox'));
				...
				"params" => array_merge(
					array(
						array(
							...
							"std" => 'contacts-labels',
							'save_always' => true,
							"value" => array_flip(apply_filters('trx_addons_sc_type', trx_addons_components_get_allowed_layouts('widgets', 'contacts'), 'trx_widget_contacts')),
							"type" => "dropdown"
						),
						array(
							"param_name" => "title",
						...
						array(
							...
							"description" => wp_kses_data( __("Name string", 'trx_addons') ),
							"admin_label" => true,
							"type" => "textfield"
						),
						array(
							"param_name" => "bdate",
							"heading" => esc_html__("Date of birth", 'trx_addons'),
							"description" => wp_kses_data( __("Date of birth string. No text formatting", 'trx_addons') ),
							"admin_label" => true,
							"type" => "textfield"
						),
						array(
						...
						array(
							"param_name" => "site",
							"heading" => esc_html__("Site", 'trx_addons'),
							"description" => wp_kses_data( __("Site string", 'trx_addons') ),
							"admin_label" => true,
							"type" => "textfield"
						),
						array(
							"param_name" => "new_window",
							"heading" => esc_html__("New Window", 'trx_addons'),
							"description" => wp_kses_data( __("Open in new window", 'trx_addons') ),
							"std" => "0",
							"value" => array("Open" => "1" ),
							"type" => "checkbox"
						),
			...
			protected function _register_controls() {
				...
				$this->add_control(
					...
					[
						...
						'options' => apply_filters('trx_addons_sc_type', trx_addons_components_get_allowed_layouts('widgets', 'contacts'), 'trx_widget_contacts'),
						'default' => 'contacts-labels'
					]
				);
				...
				$this->add_control(
					...
					[
						...
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::TEXT,
						'placeholder' => __( "Name", 'trx_addons' ),
						'default' => ''
					]
				);
				...
				$this->add_control(
					'bdate',
					[
						'label' => __( 'Date of birth', 'trx_addons' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::TEXT,
						'placeholder' => __( "Date", 'trx_addons' ),
						'default' => ''
					]
				);
				...
				$this->add_control(
					'site',
					[
						'label' => __( 'Website', 'trx_addons' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::URL,
						'placeholder' => 'http://site.example',
						'default' => array( 'url' => '')
					]
				);
