...
// Widget body
?><div class="contacts_wrap contacts_wrap_labels"><?php
...
if ($show_info || !empty($googlemap)) {
	...
	if ($show_info) {
		?><div class="contacts_info show_labels"><?php
		if (!empty($name) || !empty($bdate) || !empty($address)) {
			if ($columns) {
				?><div class="contacts_left"><?php
			}
			if (!empty($name)) {
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Name:', 'trx_addons' ) ?></span><span class="contacts_name"><?php echo str_replace('|', "<br>", $name); ?></span></div><?php
			}
			if (!empty($bdate)) {
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Date of birth:', 'trx_addons' ) ?></span><span class="contacts_bdate"><?php echo str_replace('|', "<br>", $bdate); ?></span></div><?php
			}
			if (!empty($address)) {
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Address:', 'trx_addons' ) ?></span><span class="contacts_address"><?php echo str_replace('|', "<br>", $address); ?></span></div><?php
			}
			if ($columns) {
				?></div><?php
			}
		...
		if (!empty($phone) || !empty($email) || !empty($site)) {
			...
			if (!empty($phone)) {
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Phone:', 'trx_addons' ) ?></span><a href="<?php echo esc_attr(trx_addons_get_phone_link($phone)); ?>" class="contacts_phone"><?php echo wp_kses_data($phone); ?></a></div><?php
			}
			if (!empty($email)) {
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Email:', 'trx_addons' ) ?></span><span class="contacts_email"><a href="mailto:<?php echo antispambot($email); ?>"><?php echo antispambot($email); ?></a></span></div><?php
			}
			if (!empty($site)) {
				if (is_array($site)) {
				...
				?><div class="contacts_item"><span class="contacts_label"><?php esc_html_e( 'Website:', 'trx_addons' ) ?></span><a href="<?php echo esc_url($site); ?>" class="contacts_site"<?php
