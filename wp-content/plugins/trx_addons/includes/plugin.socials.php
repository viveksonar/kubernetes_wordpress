			...
			foreach ($list as $social) {
				...
				$output .= '>'
							...
							. '>'
								...
								. ($args['type']=='list' ? '<span>' . trim($title) . '</span>' : '')
							. '</span>'
						. '</a>';
				if (!empty($url['script'])) {
...
if (!function_exists('trx_addons_add_og_tags')) {
	...
	function trx_addons_add_og_tags() {
		...
		if (is_singular() && (!isset($wp_query->is_posts_page) || $wp_query->is_posts_page!=1) && !empty($wp_query->post)) {
			...
			<meta property="og:description" content="<?php echo esc_attr( strip_tags( strip_shortcodes( has_excerpt() ? get_the_excerpt() : trx_addons_strshort($wp_query->post->post_content, 250) ) ) ); ?>" />  
