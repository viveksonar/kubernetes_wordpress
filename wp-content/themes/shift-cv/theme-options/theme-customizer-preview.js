...
( function( $ ) {
	...
	var $style = $( '#shift_cv-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		var style_container = '<' + 'style type="text/css" id="shift_cv-customizer-inline-css" />',
			$theme_css      = $( '#shift_cv-main-inline-css' );
		if ($theme_css.length == 0) {
			$theme_css = $( '#shift_cv-main-css' );
		}
		if ($theme_css.length == 0) {
		...
		$style = $( '#shift_cv-customizer-inline-css' );
	}
			...
			api.preview.bind(
				'refresh-other-controls', function( obj ) {
					...
					if (id.indexOf( 'body_style' ) == 0) {
						...
						$( '.sc_layouts_logo .logo_text' ).html( shift_cv_prepare_macros( val ) );

					} else if (id == 'blogdescription') {
						$( '.sc_layouts_logo .logo_slogan' ).html( shift_cv_prepare_macros( val ) );

					} else if (id == 'copyright') {
						$( '.copyright_text' ).html( shift_cv_prepare_macros( val ) );
						...
						if (id == 'front_page_bg_image') {
							...
							if (color_type == 'none') {
								...
								color = typeof shift_cv_color_schemes[scheme] != 'undefined' ? shift_cv_color_schemes[scheme]['colors']['bg_color'] : '';
							}
							$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : shift_cv_hex2rgba( color, mask ) ) );
							...
							.height( 'auto' )
							.toggleClass( 'shift_cv-full-height sc_layouts_flex sc_layouts_columns_middle', val );
							$( window ).trigger( 'resize' );
						}
					}
				}
