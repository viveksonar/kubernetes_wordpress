...
if ( ! function_exists( 'shift_cv_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'shift_cv_hovers_theme_setup3', 3 );
	function shift_cv_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		shift_cv_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'shift-cv' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'shift-cv' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'shift-cv' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'shift-cv' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'shift-cv' ) ),
					'std'      => 'dots',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'shift-cv' ),
					),
					'options'  => shift_cv_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'shift_cv_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'shift_cv_hovers_theme_setup9', 9 );
	function shift_cv_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'shift_cv_hovers_frontend_scripts', 1010 );
		add_filter( 'shift_cv_filter_localize_script', 'shift_cv_hovers_localize_script' );
		add_filter( 'shift_cv_filter_merge_scripts', 'shift_cv_hovers_merge_scripts' );
		add_filter( 'shift_cv_filter_merge_styles', 'shift_cv_hovers_merge_styles' );
		add_filter( 'shift_cv_filter_merge_styles_responsive', 'shift_cv_hovers_merge_styles_responsive' );
		add_filter( 'shift_cv_filter_get_css', 'shift_cv_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'shift_cv_hovers_frontend_scripts' ) ) {
	function shift_cv_hovers_frontend_scripts() {
		if ( shift_cv_is_on( shift_cv_get_theme_option( 'debug_mode' ) ) ) {
			$shift_cv_url = shift_cv_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $shift_cv_url ) {
				wp_enqueue_script( 'shift-cv-hovers', $shift_cv_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'shift_cv_hovers_merge_styles' ) ) {
	function shift_cv_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'shift_cv_hovers_merge_styles_responsive' ) ) {
	function shift_cv_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'shift_cv_hovers_localize_script' ) ) {
	function shift_cv_hovers_localize_script( $arr ) {
		$arr['button_hover'] = shift_cv_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'shift_cv_hovers_merge_scripts' ) ) {
	function shift_cv_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'shift_cv_hovers_add_icons' ) ) {
	function shift_cv_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr( $no_link ); ?>" <?php shift_cv_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr( $no_link ); ?>" <?php shift_cv_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr( $no_link ); ?>" <?php shift_cv_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php shift_cv_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
					...
					<div class="post_descr">
						<?php
						if ( $hover === 'fade' ) {
							$shift_cv_components = 'categories';
							$shift_cv_counters   = array();
						} else {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php shift_cv_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'shift_cv_hovers_get_css' ) ) {
	function shift_cv_hovers_get_css( $css, $args ) {
...
.post_featured.hover_fade .post_info .post_meta_item a,
